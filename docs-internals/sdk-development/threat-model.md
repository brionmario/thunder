...
server process belonging to a third party, and it is responsible for starting an
authentication flow, completing it, holding the resulting tokens, attaching them to outbound
requests, and clearing them on sign-out. Everything the SDK does is observable, and on a
compromised device modifiable, by the runtime it is embedded in. Its security value is
therefore not secrecy of execution but correctness of protocol: binding the authorization
request to the callback, keeping tokens in the strongest store the platform offers, never
retaining a credential, and never widening what an attacker who reaches the runtime can do.
...
| 4 | [Security Risk] | An SDK offers a configuration key that disables PKCE or state, and an application enables it | [No] | Both are specified as mandatory and not configurable. Any key that would weaken them requires a change to this model. An `enablePKCE` key exists in the JavaScript configuration today and is tracked as a residual below |
...
- A configuration key that can disable PKCE exists in the JavaScript SDK configuration
  (`enablePKCE`), which contradicts PKCE being mandatory in redirect mode. The key needs to be
  removed, or narrowed to a non-production affordance that cannot apply against a
  non-loopback `baseUrl` (tracking:
...
- Offline sign-out behaviour is unspecified: whether the SDK clears local state when
  revocation cannot be reached is left to the implementation, and differs today.
...
- The shared error code catalogue referenced in
  [spec.md](spec.md#error-model) does not exist yet, so an application cannot yet branch on a
  code uniformly across SDKs.
